...
class EmbeddingStem(nn.Module):
	...
	def forward(self, x):
		...
		if seq_len != self.pos_embed.size(1):  # 如果序列长度不同，调整 pos_embed
			pos_embed_resized = torch.nn.functional.interpolate(
				self.pos_embed[:, :self.pos_embed.size(1), :].permute(0, 2, 1),  # 调整维度顺序
				size=seq_len,
				mode='linear',
				align_corners=False
			).permute(0, 2, 1)  # 恢复原始维度
		else:
			pos_embed_resized = self.pos_embed
		# print(f"x shape: {x.shape}, pos_embed shape: {pos_embed_resized.shape}")
		return self.pos_drop(x + pos_embed_resized)
