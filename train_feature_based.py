...
def train_feature_based(data_path, classifier_name, split_per=0.7, seed=None, read_from_file=None, eval_model=False, path_save=None,self_save_done_training=None):
	# Set up
	...
	if self_save_done_training is None:
		df.to_csv(os.path.join(save_done_training, f"{classifier_name}_{timestamp}.csv"))
	else:
		df.to_csv(os.path.join(self_save_done_training, f"{classifier_name}_{timestamp}.csv"))
